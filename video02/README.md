...
8-正则化(regularization)
    过拟合的问题
        如果有非常多的特征，通过学习得到的假设可能会非常好的适应训练集（代价函数可能几乎为0），但是不能推广到新的数据。
        例：对某些数据，线性模型：欠拟合，不能很好适应；
            四次方模型：过于强调拟合原始数据，不能很好预测新数据；
            中间的模型似乎最合适。
        以多项式来理解，x的次数越高，拟合的越好，但相应的预测能力就可能变差。
        如何处理？
            丢弃一些不能帮助我们正确预测的特征，可以手工保留，或者使用模型算法（例如PCA）
            正则化：保留所有特征，但是减少参数的大小。（加入惩罚）

    代价函数
        拟合函数中高次项导致了过拟合的产生，最好使高次项系数接近于0来更好的拟合。
        正则化基本方法：
            修改代价函数，在函数高次项参数设置一点惩罚。
            注：正则化参数要考虑取一个合理的值
...
9-神经网络学习：表述
    非线性假设
        使用线性回归和逻辑回归，当特征太多时，计算的负荷会非常大。
        使用非线性的多项式项，能够建立更好的分类模型。
        普通的逻辑回归模型，不能有效处理众多的特征，这时候需要神经网络。

    神经元与大脑
        神经网络很古老，最初产生的目的是制造能模拟大脑的机器。
        对于五花八门的事情，大脑处理的方法，只需要一个单一的学习算法就可以了！
        例：听觉皮层学会“看”
            舌头学会“看”   人体回声定位、人体声呐
            触觉皮带
            青蛙学习使用第三只眼
    ...
    模型展示I
        神经元--处理单元/神经核
            含有许多输入/树突，有一个输出/轴突
            神经网络是大量神经元相互链接并通过电脉冲突来交流的一个网络
            神经网络模型建立在很多神经元之上，每一个神经元又是一个个学习模型。
            这些神经元（激活单元）采纳一些特征作为输入，并且根据本身的模型提供一个输出。
            在神经网络中，参数又可被称为权重（weight）。
            神经网络模型是许多逻辑单元按照不同层级组织起来的网络，每一层的输出变量都是下一层的输入变量。
            例：3层的神经网络，第一层称为输入层，最后一层称为输出层，中间一层称为隐藏层。
